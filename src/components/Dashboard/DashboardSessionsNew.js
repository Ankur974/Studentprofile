...
const DashboardSessions = ({
  ...
  const { user, dropDownStatus, bseTools, provider } = useSelector(state => ({
  ...
  const checkIfNewToolsAssigned = () =>
    dispatch(fetchBseTools(providerType, "pending", 20));
      ...
      <Section>
        ...
        >
          <FlexBox column justify="space-between" noPosition="unset">
            <TextContainer style={{ backgroundColor: !isMobile && ACCENT_200 }}>
            ...
            {/* TODO : Implement logic for below */}
            {/* <FirstSessionCard
              providerPublicProfile={providerPublicProfile}
        ...
        <CardRight className="hide-scrollbar">
          ...
          {(completedSessions.length > 0 || providertype !== "coach") && (
            <SessionCredits
              newSessionCredits={newSessionCredits}
          ...
          {user?.usertype === "patient" &&
            !!providertype &&
            providertype !== COUPLE_THERAPIST && (
              <ReferralCard providerType={providertype} />
            )}
        </CardRight>
      </Section>
